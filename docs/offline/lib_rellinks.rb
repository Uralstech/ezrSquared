...
def relativize(href, path, absolute_base, root_dir)
  ...
  href_url = URI.join(URI.escape(absolute_base), URI.escape(href))
  path_url = URI.join(absolute_base, URI.escape(path))
...
def test_index(relative_url, href_url, absolute_base, root_dir)
  if path_is_dir(href_url, absolute_base, root_dir)
    ...
    test_url = URI.join(URI.escape(absolute_base), URI.escape(fixed_url))
